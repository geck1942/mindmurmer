...
class MindMurmurBaseSoundController(object):
	TRACKS_KEY = "tracks"

	def __init__(self, audio_folder, sample_rate):
...
class MindMurmurSoundScapeController(MindMurmurBaseSoundController):
	""" Non-blocking Audio controller to mix tracks and play sound scapes according to input meditation stage
	...
	def _validate_audio_files_and_prep_data(self):
		...
		for dir_name in os.listdir(self.audio_folder):
			...
			if len(tracks) == 0:
				...
				logging.info("Got stage \"{stage_number}\"stage_number".format(stage_number=stage_number))
	...
	def set_meditation_stage(self, stage):
		stage_track = self._get_meditation_stage_data_for_stage(stage)
		self._mix_track(stage_track)


class MindMurmurHeartbeatAudioController(MindMurmurBaseSoundController):
	...
	def __init__(self, audio_folder, sample_rate=16000):
		...
		super(MindMurmurHeartbeatAudioController, self).__init__(audio_folder, sample_rate)
	...
	def _validate_audio_files_and_prep_data(self):
		...
		for dir_name in os.listdir(self.audio_folder):
			...
			if heartbeat_sound_filename is None:
	...
	def _play_heartbeat(self, heartbeat_track):
		...
		self.playing_tracks.append(heartbeat_channel)
	...
	def _get_meditation_stage_data_for_rate(self, rate):
	...
	def set_heart_rate(self, rate):
		...
		"""
		stage_data = self._get_meditation_stage_data_for_rate(rate)
...
if __name__ == "__main__":
	...
	parser.add_argument('--mode', choices=["heartbeat", "soundscape"], dest='mode', help="testing mode")
