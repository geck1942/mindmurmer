...
class RabbitController(object):
    ...
    def _base_subscribe(self, consume_target_str, queue_name, callback):
        try:
            self.active_channel.queue_declare(queue=queue_name)
            self.active_channel.basic_consume(callback, queue=queue_name, no_ack=True)
    ...
    def start_consuming(self):
        if self.active_channel is not None:
            ...
            logging.info("No open channel to consume from!")
            raise ValueError("No open channel to consume from!")
