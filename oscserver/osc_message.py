...
class OscMessage(object):
            ...
            for param in type_tag:
                if param == "i":    # Integer.
                    ...
                    val, index = get_midi(self._dgram, index)
                        ...
                        val, index = get_ttag(self._dgram, index)
                elif param == "T":    # True.
                    val = True
                elif param == "F":    # False.
                    val = False
                    ...
                    if len(param_stack) < 2:
                        raise ParseError(
                          ...
                          format(type_tag))
                    param_stack.pop()
                # TODO: Support more exotic types as described in the spec.
                else:
                    logging.warning('Unhandled parameter type: {0}'.
                                    format(param))
                    continue
                if param not in "[]":
                    param_stack[-1].append(val)
            if len(param_stack) != 1:
                raise ParseError('Missing closing bracket in type tag: {0}'.
                                 format(type_tag))
    ...
    def dgram_is_message(dgram):
        ...
        print("is_message", dgram.startswith(b'/'))
